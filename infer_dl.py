...
#CUDA_VISIBLE_DEVICES=0,1,3,6 python infer_dl.py --name recon_dl718 --infer_list voc12/val.txt --dict --crf --load_epo 22
...
if __name__ == '__main__':
    ...
    for iter, pack in enumerate(tqdm(infer_data_loader)):
        ...
        if args.dict:
            if phase =='val':
                model.infer_single(42, infer_path, dict_path, crf_path, vis=False, dict=True, crf=False) #Val
            elif phase =='test':
                model.infer_single(42, test_path, dict_path, crf_path, vis=False, dict=True, crf=False)  # test
