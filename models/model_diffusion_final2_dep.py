...
class model_WSSS():
    ...
    def update(self, epo):
        ...
        _rcams = self.max_norm(rcams).detach()
        
        if epo>9:
            self.loss_cam_consistency = self.args.W[0]*(
                ((_cam_diff-_rcams.detach())*self.label.view(B,C,1,1)).abs().mean()
            )
        else:
            self.loss_cam_consistency = self.args.W[0]*(
        ...
        if self.args.W[2]>0:
            self.loss_rcams_consistency = self.args.W[2]*(
                ((_rcams-self.max_norm(rcams_diff))*self.label.view(B,C,1,1)).abs().mean()
            )
