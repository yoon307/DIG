...
class model_WSSS():
    ...
    def __init__(self, args, logger):
        ...
        self.base_names = ['pred_diff','cls','cls_diff','cam_consistency','feat_consistency']
        ...
        self.net_diffcls = resnet38d.Net_diff(D=384,C=20)
        ...
        if args.finetune:
            ...
            if args.finetune.startswith('https') and 'MCTformer' in args.trm:
                ...
                new_cls_token = cls_token_checkpoint.repeat(1,args.C,1)#+ 0.2*perturb*sign
    ...
    def train_setup(self):
        ...
        param_diffcls = self.net_diffcls.get_parameter_groups()
        # self.opt_diffcls = utils.PolyOptimizer_adam([
        #         {'params': param_diffcls[0], 'lr': 1 * args.lr_cnn, 'weight_decay': args.wt_dec_cnn},
        #         {'params': param_diffcls[1], 'lr': 2 * args.lr_cnn, 'weight_decay': 0},  # non-scratch bias
        #         {'params': param_diffcls[2], 'lr': 10 * args.lr_cnn, 'weight_decay': args.wt_dec_cnn},  # scratch weight
        #         {'params': param_diffcls[3], 'lr': 20 * args.lr_cnn, 'weight_decay': 0}  # scratch bias
        #     ],
        #         lr=args.lr_cnn, weight_decay=args.wt_dec_cnn, max_step=args.max_step)
        self.opt_diffcls = torch.optim.AdamW(self.net_diffcls.parameters(),self.args.lr_cnn,weight_decay=0)
        ...
        self.net_trm = torch.nn.DataParallel(self.net_trm.to(self.dev))
        self.net_diffcls = torch.nn.DataParallel(self.net_diffcls.to(self.dev))
    ...
    def gndp2d(self, x, valid_cat, keepdims=False, score_th=0.5, is_RIB=False):
    ...
    def raw_logit_loss_supp_others_norm(self, logits, labels):
    ...
    def update(self, epo):
        ...
        self.net_diffcls.train()

      
        self.img_norm = F.interpolate(self.denormforDiff(self.img),size=(192,192),mode='bilinear',align_corners=True)
        self.img_norm = self.diffusion.normalize(self.img_norm) # -1 ~ +1

        # self.img_norm05 = F.interpolate(self.denormforDiff(self.img),size=(96,96),mode='bicubic',align_corners=True)
        # self.img_norm05 = self.diffusion.normalize(self.img_norm05) # -1 ~ +1

        #################diffusion process###############
        # step = self.args.step * min(epo//3+1,4)
        step = self.args.step

        # Create the Gaussian kernel for each image in the batch

        # TFG = torchvision.transforms.GaussianBlur(5,(0.1,2))
        # img_diff = TFG(self.img)


        feat_diff_sum = torch.zeros((B,2432,24,24)).cuda()
        ...
        with torch.no_grad():
            for i in range(split):
                ...
                for j in range(step[0],step[1],step[2]):
                    ...
                    # _, _, feat_diff05 = self.diffusion.p_losses(self.img_norm05[slice*i:slice*(i+1)],t)

                    img_out = self.diffusion.predict_start_from_v(x_t,t,v)
                    img_out = self.diffusion.unnormalize(img_out)
                    
                    # x_t = self.diffusion.q_sample(self.img_norm,t)
                    # x_t = self.diffusion.unnormalize(x_t) #0~1

                    # img_out = self.diffusion.sample_from_t(B, self.img_norm, step)
                    
                    H,W = self.img.size()[2:]
                    img_out = F.interpolate(img_out,size=(H,W),mode='bicubic',align_corners=True)
                    img_diff = self.normforCls(img_out)

                    feat_diff = torch.cat(feat_diff[:-1],dim=1) 
                    # feat_list.append(feat_diff)
                    feat_diff_sum += feat_diff

                    # feat_diff05 = torch.cat(feat_diff05[:5],dim=1) # 512/512/512/512/256/256/256/128/128/128/64/64/64/64
                    # feat_list05.append(feat_diff05)
                    # 512/512/512/256/256/128/128/64/64/64

            
        self.opt_diffcls.zero_grad()

        # feat_stack = torch.stack(feat_list,dim=0).sum(0)
        # feat_stack05 = torch.stack(feat_list05,dim=0)

        # cam_diff, feat_diff, pred_diff = self.net_diffcls(feat_stack.detach())
        # cam_diff05, feat_diff05, pred_diff05 = self.net_diffcls(feat_stack05.detach())

        # _cam_diff = self.max_norm(cam_diff)
        # _cam_diff05 = F.interpolate(self.max_norm(cam_diff05),size=cam_diff.size()[2:],mode='bilinear',align_corners=False)
        
        # self.loss_pred_diff = 0.01*self.raw_logit_loss_supp_others_norm(self.gndp2d(cam_diff,self.label,keepdims=True,is_RIB=True,score_th=0.8).view(B,C),self.label)[0]
        # + F.multilabel_soft_margin_loss(pred_diff,self.label))

        # self.loss_pred_diff = F.multilabel_soft_margin_loss(pred_diff,self.label)
        ...
        # for i in range(B):
        #     gt = self.label[i]
        #     gt_len = torch.nonzero(gt).size(0)
        #     x = feat_stack[i].view(2432,-1).permute(1,0)

        #     cluster_ids_x, cluster_centers = kmeans(
        #     X=x, num_clusters=5, distance='euclidean', device=torch.device('cuda')
        #     )

        #     cluster_ids_x = F.interpolate(cluster_ids_x.reshape(1,1,24,24).float(),size=(H,W),mode='nearest')[0,0]

        #     input = denorm(self.img[i]).permute(1,2,0)
        #     plt.imshow(input.cpu().detach().numpy())
        #     plt.imshow(cluster_ids_x.cpu().detach().numpy(), cmap='jet', alpha=0.4)
        #     plt.savefig("./%s/cluster%d.png"%(self.args.val_path,i))
        ...
        self.loss_pred_diff.backward()
        self.opt_diffcls.step()
        

        input = denorm(self.img[0]).permute(1,2,0)
        cam_diff = F.relu(F.interpolate(cam_diff,size=(H,W),mode='bilinear',align_corners=False))*self.label.view(B,C,1,1)
        # cam_diff05 = F.relu(F.interpolate(cam_diff05,size=(H,W),mode='bilinear',align_corners=False))*self.label.view(B,C,1,1)
        ...
        patch_attn_S = outputs['attn_S']
        rcams = outputs['rcams']
        self.feat_diff = outputs['feat_diff']
        ...
        # self.loss_pred_diff =  self.L1(pred_diff,self.out_patch.detach())+ self.bce(pred_diff,self.label)
        # self.loss_pred_diff.backward()
        # self.opt_diffcls.step()
        
        # plt.imshow(denorm(self.img[0]).permute(1,2,0).detach().cpu().numpy())
        # plt.savefig("temp.png")
        # plt.close()

        # plt.imshow(img_out[0].permute(1,2,0).detach().cpu().numpy())
        # plt.savefig("temp2.png")
        # plt.close()

        # plt.imshow(x_t[0].permute(1,2,0).detach().cpu().numpy())
        # # plt.imshow(denorm(img_diff[0]).permute(1,2,0).detach().cpu().numpy())
        ...
        if self.args.W[1]>0 or self.args.W[2]>0:
            ...
            patch_attn_S_diff = outputs_diff['attn_S']
        ...
        if self.args.W[3]>0:
            # _cams = self.max_norm(F.interpolate(cams,size=norm_cam.size()[2:],mode='bilinear',align_corners=False))
            
            self.feat_diff = F.interpolate(self.feat_diff,size=feat_diff.size()[2:],mode='bilinear',align_corners=False)
            self.feat_diff  = F.normalize(self.feat_diff,dim=1)
            
            # feat_diff05 = F.interpolate(feat_diff05,size=feat_diff.size()[2:],mode='bilinear',align_corners=False)
            # feat_diff = (feat_diff + feat_diff05)/2
            feat_diff = F.normalize(feat_diff,dim=1)

            self.loss_feat_consistency = self.args.W[3]*(
                # ((norm_cam.detach()-_cams)*self.label.view(B,C,1,1)).abs().mean()
                # (feat_diff.detach()-self.feat_diff)
                self.L1(self.feat_diff.type(torch.FloatTensor), feat_diff.type(torch.FloatTensor).detach())
            )
            loss_trm += self.loss_feat_consistency
        else: 
            self.loss_feat_consistency = torch.Tensor([0])
        ##############################################
        ...
        self.count_rw(self.label, pred_diff, 0)
        self.count_rw(self.label, self.out_patch, 1)
