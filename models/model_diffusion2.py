...
##########
# gpu0 python train_trm.py --name diff --model diffusion2 --W 1 1 1 --step 0 5 1 --T 4 --A 0.4
...
class model_WSSS():
    ...
    def update(self, epo):
        ...
        # self.label_with_fg = torch.cat([self.label,torch.ones((B,1).cuda())],dim=1)

        #################diffusion process###############
        step = self.args.step

        # feat_diff_sum = torch.zeros((B,512,28,28)).cuda()
        feat_diff_list = []
        ...
        with torch.no_grad():
            for i in range(split):
                ...
                for j in range(step[0],step[1],step[2]):
                    ...
                    # feat_diff0 = torch.cat([feat_diff[0],feat_diff[1]],dim=1) 
        ...
        # self.cam_diff_cs = outputs['cam_diff_cs']
        # self.pred_diff_cs = outputs['pred_diff_cs']
        ...
        #     print(self.label[0])
        #     print(F.softmax(self.out_patch.detach()/T,dim=1)[0])
        #     print(F.softmax(self.out_patch.detach()/T,dim=1)[0].max())
        #     print(F.softmax(self.out_patch.detach()/T,dim=1)[0].min())

        self.loss_pred_diff = self.args.W[1]*(
            alpha* F.multilabel_soft_margin_loss(self.pred_diff,self.label)
            # + (1-alpha)*(T*T)*(1/B)*(self.KD(F.log_softmax(self.pred_diff/T,dim=1),F.softmax(self.out_patch.detach()/T,dim=1))*self.label).sum() 
        ...
        # cam_sum = (self.max_norm(self.cam_diff)+self.max_norm(self.cam_diff_cs))/2

        # self.cam_diff_cs = self.max_norm(self.cam_diff_cs)*self.label.view(B,C,1,1)
        self.cam_diff = self.max_norm(self.cam_diff)*self.label.view(B,C,1,1)

        # _feat_diff = F.normalize(feat_diff_list[0].view(B,512,-1),dim=1) # B 1024 HW
        # _feat_diff_attn = torch.bmm(_feat_diff.permute(0,2,1),_feat_diff)
        
        # _feat_diff_trm = F.normalize(self.feat_diff_trm.view(B,384,-1),dim=1) # B 1024 HW
        # _feat_diff_trm_attn = torch.bmm(_feat_diff_trm.permute(0,2,1),_feat_diff_trm)

        # self.feat = F.interpolate(self.feat,size=self.feat_diff.size()[2:],mode='bilinear',align_corners=False) 

        cam_diff_sum = (self.cam_diff).detach()

        self.loss_cam_consistency = self.args.W[2]*(
            # (F.normalize(self.feat,dim=1)-F.normalize(self.feat_diff,dim=1).detach()).abs().mean()
            ((cam_diff_sum-_cams)*self.label.view(B,C,1,1)).abs().mean()
            # + (_feat_diff_attn.detach()-_feat_diff_trm_attn).abs().mean()
        )
        if epo>8:
        ...
        norm_cam = F.interpolate(cam_diff_sum,size=(H,W),mode='bilinear',align_corners=True) 
        ...
        # if self.args.W[2]>0 or self.args.W[3]>0:
        ...
        #     self.loss_cls_diff = self.args.W[2]*(
        ...
        # if self.args.W[3]>0:
        #     self.loss_cam_consistency = self.args.W[3]*(
        ...
        #     feat_diff_sum = F.normalize(feat_diff_sum,dim=1)

        #     self.loss_feat_consistency = B*self.args.W[3]*(
        #         self.L2(self.feat_diff.type(torch.FloatTensor), feat_diff_sum.type(torch.FloatTensor).detach())
        #     )
