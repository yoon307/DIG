...
##########
# gpu0 python train_trm.py --name diff --model diffusion_real_final --W 1 1 1 1 5 --T 5 --A 0.2 
...
class model_WSSS():
    ...
    def __init__(self, args, logger):
        ...
        self.net_trm = create_model(
            # 'deit_small_MCTformerV2_diff_hy_sh_patch16_224',
            'deit_small_MCTformerV2_diff_V2_patch16_224',
            pretrained=False,
            num_classes=args.C,
            drop_rate=args.drop,
        ...
        ##EMA##
        self.diffusion_ema = EMA(self.diffusion, beta = 0.995, update_every = 10)
        self.diffusion_ema.load_state_dict(unet_state_dict['ema'])
        self.diffusion_ema.ema_model.eval()
        self.diffusion_ema.ema_model.training = False
        ...
        if args.finetune:
            ...
            if args.finetune.startswith('https') and 'MCTformer' in args.trm:
                ...
                # checkpoint_model['cls_token'] = cls_token_checkpoint        # If use tokenizer
    ...
    def update(self, epo):
        ...
        # self.img_norm = self.denormforDiff(F.interpolate(self.img,size=(192,192),mode='bicubic',align_corners=True))
        ...
        if (self.args.W[1]>0 or self.args.W[2]>0) or self.args.W[3]>0:
            with torch.no_grad():
                ...
                # for j in [60]:
                    ...
                    if j == 200 :
                        img_out = self.diffusion_ema.ema_model.ddim_sample_from_t((B,H,W),self.img_norm,j)
                        # img_out = self.diffusion_ema.ema_model.predict_start_from_v(x_t,t,v)
                        # img_out = self.diffusion.unnormalize(img_out)

                        H,W = self.img.size()[2:]
                        img_out = F.interpolate(img_out,size=(H,W),mode='bicubic',align_corners=True)
                        
                        img_vis_from_diff = torch.clip(img_out[0],0,1)
                        save_img("temp2.png",img_vis_from_diff.permute(1,2,0).detach().cpu().numpy())
        ...
        if self.args.W[1]>0:
            ...
            self.loss_pred_diff = self.args.W[1]*(
                ...
                ###################SOFTMAX###############
                # + (1-alpha)*(T*T)*self.KD(F.log_softmax(self.diff_cls/T,dim=1),F.softmax(self.out.detach()/T,dim=1))
                # + (1-alpha)*(T*T)*self.KD(F.log_softmax(self.diff_pcls/T,dim=1),F.softmax(self.out_patch.detach()/T,dim=1))   # original
                
                # + (1-alpha) * (T*T) * self.KD_log(S_pred, T_pred_patch)
                # + (1-alpha) * (T*T) * self.KD_log(S_pred, T_pred_ctk)

                + (1-alpha) * (T*T) * self.KD_log(S_pred_ctk_cat, T_pred_ctk_cat) * 0.5
                + (1-alpha) * (T*T) * self.KD_log(S_pred_patch_cat, T_pred_patch_cat) * 0.5

                # + (1-alpha) * (T*T) * ((self.KD_none(torch.log(S_pred), T_pred_patch) + self.KD_none(torch.log(1 - S_pred), 1 - T_pred_patch)).sum() / B) * 0.5
                # + (1-alpha) * (T*T) * ((self.KD_none(torch.log(S_pred), T_pred_ctk) + self.KD_none(torch.log(1 - S_pred), 1 - T_pred_ctk)).sum() / B)
            )
        ...
        ##############################################
        if (self.args.W[3]>0 or self.args.W[4]>0) and epo>2:
