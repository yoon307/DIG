...
from networks.resnet38d import ResBlock
import copy

class Attention_with_CAM(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., num_classes=20):
        super().__init__()
        self.num_classes = num_classes
        self.num_heads = num_heads
    ...
    def forward(self, x, CAM=None):
        ...
        ###############################################################
            ...
            # x_tilda[:,20:,:] = x[:,20:,:]*(F.softmax(CAM,dim=1))
            # q, k, v = qkv[0], qkv[1], qkv[2] #B H N C//head

            _qkv = self.qkv(_x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            # k,v = kv[0], kv[1]
            q, k, v = _qkv[0], _qkv[1] , _qkv[2]

            
 
        
        attn_all = (q @ k.transpose(-2, -1)) * self.scale  # B x num_heads x N x N
        ...
        # return x, weights ##original
        return x_final, weights, q
        # return x_conv, weights_conv, v
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        # return x, weights ##original
        return x, weights, qkv, o

from networks.resnet101 import ResBlock_ysh, Bottleneck
class Block_ysh(nn.Module):

    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, num_classes=20):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention_with_CAM(
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, num_classes=num_classes)
    ...
    def forward(self, x, size=None):
        ...
        o, weights, value = self.attn(self.norm1(x),size)
        x = x + self.drop_path(o)
        x = x + self.drop_path(self.mlp(self.norm2(x)))
       
            
        return x, weights, value
...
class VisionTransformer(nn.Module):
    ...
    def _init_weights(self, m):
        ...
        # elif isinstance(m, nn.Conv2d): #ADDED BY YSH
...
class VisionTransformer_ysh(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., norm_layer=nn.LayerNorm, mask_type=None):
        super().__init__()
